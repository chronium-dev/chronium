...
import { member, template, workspace } from '$lib/server/db/schema';
...
export const load: PageServerLoad = async ({ locals }) => {
	...
	const result = await db
		...
		.from(template)
		.innerJoin(workspace, eq(template.wsId, workspace.id))
		.innerJoin(member, eq(workspace.id, member.wsId))
