...
import type { RatingStatus } from '$lib/types/ratings';
import type { BasicUserContext } from '$lib/types/User';
import { and, asc, desc, eq, gte, inArray, isNotNull, lte, sql } from 'drizzle-orm';
import { PlanTypes } from '../../types/PlanTypes';
import { db } from './index';
import { member, rating, ratingReply, template, user, workspace } from './schema';
...
export async function isUserWorkspaceOwner(userId: string, workspaceId: string): Promise<boolean> {
	const result = await db
		...
		.from(member)
		.where(
			and(eq(member.userId, userId), eq(member.workspaceId, workspaceId), eq(member.role, 'owner'))
		)
		.limit(1);
