...
import { generateCommonObligationsController } from '$lib/server/process/common/generateCommonObligationsController';
import { generateStatutoryObligationsController } from '$lib/server/process/statutory';
 ...
 * @param userId
 */
export async function generateAndPersistObligations(
	org: Organisation,
	userId: string,
	tx?: DBExecutor
) {
	...
	// Execute the Statutory generations
	const generatedStatutoryObligationDates = generateStatutoryObligationsController(
		org,
	...
	const generatedObligationDates = generatedStatutoryObligationDates.concat(
		generatedCommonObligationDates
	);
